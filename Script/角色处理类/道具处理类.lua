...
function 道具处理类:一键回收(连接id,数字id) --羔羊回收
	for n=1,20 do
		if 玩家数据[数字id].角色.道具[n] and self.数据[玩家数据[数字id].角色.道具[n]] then
			local 道具id = 玩家数据[数字id].角色.道具[n]
...
---------------------------------------------------------------------------------------
    ...
    玩家数据[id].道具:给予道具(id,"星辉石",math.random(1,4))
		道具使用=true
	elseif 名称 == "中级星辉石礼包" then
    玩家数据[id].道具:给予道具(id,"星辉石",math.random(4,8))
		道具使用=true
	elseif 名称 == "高级星辉石礼包" then
    玩家数据[id].道具:给予道具(id,"星辉石",math.random(6,10))
		道具使用=true
...
elseif 名称 == "星辉石max" then
  ...
  elseif 名称 == "超级鲲鹏礼盒（法术）" then
	...
	elseif 名称=="修炼果礼包" then
		玩家数据[id].道具:给予道具(id,"修炼果",取随机数(2,5))
		道具使用=true
	elseif 名称=="陨铁礼包" then
		local 道具格子=玩家数据[id].角色:取道具格子2()
		if 道具格子<1 then
...
function 道具处理类:自动回收道具(id,道具id) --羔羊回收
	  if self.数据[道具id] == nil then return end
		...
		if self.数据[道具id].总类==5 and self.数据[道具id].分类==6 and self.数据[道具id].名称~="陨铁" and self.数据[道具id].名称~="精致碎石锤" and self.数据[道具id].名称~="钨金" and self.数据[道具id].名称~="战魄" then --宝石、星辉
				...
				if ss文字=="不回收" then return end
				local djdj = self.数据[道具id].级别限制 or 1
				if ss文字~="全部回收" then
					local abc = string.gsub(ss文字,"级以下","")
					abc = abc + 0
					if djdj > abc then
						return
					end
				...
				if self.数据[道具id].名称 == "星辉石" then
					if Q_选项回收价格.星辉 and Q_选项回收价格.星辉[djdj] ~= nil then
						获得银子 = Q_选项回收价格.星辉[djdj]
						玩家数据[self.数字id].角色:添加银子(获得银子,"回收")
					end
				else
				  if Q_选项回收价格.宝石 and Q_选项回收价格.宝石[djdj] ~= nil then
						获得银子 = Q_选项回收价格.宝石[djdj]
						玩家数据[self.数字id].角色:添加银子(获得银子,"回收")
					end
				end
				提示=true
		...
		elseif self.数据[道具id].总类==2 and self.数据[道具id].分类 >= 1 and self.数据[道具id].分类 <= 6 then --装备
			  local ss文字 = data.装备
				...
				提示=true
		elseif self.数据[道具id].总类==2 and self.数据[道具id].分类 >= 7 and self.数据[道具id].分类 <= 9 then --bb装
			  local ss文字 = data.宝宝装备
				if ss文字=="不回收" or self.数据[道具id].专用 or not self.数据[道具id].鉴定 then return end
				if self.数据[道具id].精魄等级 ~= nil and self.数据[道具id].精魄等级 >= 5 then return end
				local djdj = self.数据[道具id].级别限制 or 25
				if ss文字~="全部回收" then
					local abc = string.gsub(ss文字,"级以下","")
					abc = abc + 0
					if djdj > abc then
				...
				if Q_选项回收价格.宝宝装备[djdj] ~= nil then
					获得银子 = Q_选项回收价格.宝宝装备[djdj]
					玩家数据[self.数字id].角色:添加银子(获得银子,"回收")
				end
				提示=true
		...
		elseif self.数据[道具id].名称=="制造指南书" then
			  local ss文字 = data.装备书
				...
				if Q_选项回收价格.装备书[djdj] ~= nil then
					获得银子 = Q_选项回收价格.装备书[djdj]
					玩家数据[self.数字id].角色:添加银子(获得银子,"回收")
				end
				提示=true
		elseif self.数据[道具id].名称=="百炼精铁" then
			  local ss文字 = data.百炼精铁
				if ss文字=="不回收" then return end
				local djdj = self.数据[道具id].子类 or 10
				if ss文字~="全部回收" then
					local abc = string.gsub(ss文字,"级以下","")
		...
		elseif self.数据[道具id].名称=="灵饰指南书" then
			  local ss文字 = data.灵饰书
				if ss文字=="不回收" then return end
				local djdj = self.数据[道具id].子类 or 60
				if ss文字~="全部回收" then
					local abc = string.gsub(ss文字,"级以下","")
					abc = abc + 0
					if djdj > abc then
