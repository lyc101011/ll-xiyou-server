...
-- @Last Modified time: 2025-04-13 13:45:51
local 装备处理类 = class()
	...
	if 分类 == "打造" then
		if 功能 == "强化人物装备" then
			...
			if not 元神打造 then
				...
				常规提示(对方id,"打造熟练度增加300")
				if 玩家数据[id].打造方式 == 1 then
					玩家数据[对方id].角色:增加生活技能熟练("打造技巧",300)
				elseif 玩家数据[id].打造方式 == 2 then
					玩家数据[对方id].角色:增加生活技能熟练("裁缝技巧",300)
				end
...
function 装备处理类:生成打造装备假人(id,级别,种类,名称,生产方式,制造者,是否鉴定,专用,无级别,对方id,返回,sld) --真正的强化打造公式
	-- (id,等级*10,武器序列,武器名称,"系统产出",nil,true,id,无级别)
	local dz = self:装备万能公式(级别,种类,生产方式,专用,对方id, nil, sld)
	local 道具 = 物品类()
	道具:置对象(名称,nil,nil,级别)
	道具.级别限制 = 级别
	if 种类 ~= nil then
...
function 装备处理类:装备万能公式(a,b,生产方式,专用,对方id,双加,sld) -- 制作书等级 制作种类  --真正的强化打造公式
	local 附加属性 = false
	local c = {} -- 命中 伤害 防御 魔法 灵力 气血 敏捷 体质 力量 耐力 魔力 附加特效 附加特技 五行 耐久 鉴定
	if a == nil or b == nil then
		return false
	...
	local gl=取随机数(1,2+math.min(math.ceil(熟练度/1000),60)) --最高36，保底6
	if tx <= gl then
		local txb = {}
		local sj = random(1,#通用特效)
		txb[#txb+1]=通用特效[sj]
...
function 装备处理类:取160装备属性(制造种类,元身属性,id,打造id)
	...
	SLD=取随机数(1,2+math.min(math.ceil(SLD/1000),60)) --最高63，保底3
