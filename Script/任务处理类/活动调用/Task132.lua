...
__GWdh111[132]=function(连接id,数字id,序列,标识,地图)
	local 对话数据={}
		对话数据.模型=任务数据[标识].模型
		对话数据.名称=任务数据[标识].名称
...
__GWdh222[132]=function (连接id,数字id,序号,内容)
	local 事件=内容[1]
	local 名称=内容[3]
	if 事件=="讨教一二" then
...
function 设置任务132()
	local 任务id=取随机数(11,88).."_132_"..os.time().."_"..随机序列.."_"..取随机数(88,99999999)
	随机序列=随机序列+1
