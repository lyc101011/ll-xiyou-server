...
-- __GWdh111[213]=function(连接id,数字id,序列,标识,地图)
-- 	local 对话数据={}
--   	对话数据.模型=任务数据[标识].模型
--   	对话数据.名称=任务数据[标识].名称
--   	if 任务数据[标识].zhandou==nil then
--       对话数据.对话="喜迎新春大吉大利，享好运"
--       对话数据.选项={"驱年兽过大年","我再准备准备"}
--     else
--       对话数据.对话="我正在战斗中，请勿打扰。"
--     end
-- 	return 对话数据
...
-- __GWdh222[213]=function (连接id,数字id,序号,内容)
--   local 事件=内容[1]
--   local 名称=内容[3]
--   if 事件=="驱年兽过大年" then
...
--     战斗准备类:创建战斗(数字id+0,100099,玩家数据[数字id].地图单位.标识)
...
--   x待发送数据[地图]={}
--   for i=1,数量 do
--     local 任务id="_"..i.."_"..取随机数(11,88).."_213_"..os.time().."_"..随机序列.."_"..取随机数(88,99999999)
--     随机序列=随机序列+1
--     local xy=地图处理类.地图坐标[地图]:取随机点()
--     任务数据[任务id]={
--       id=任务id,
...
--   end
--   x待发送数据={}
--   广播消息({内容=format("#W/一群年兽出现在了#G/%s#W/处捣乱，还请各位英雄侠士赶紧去降服它们。",取地图名称(地图)),频道="xt"})
-- end

-- function rwgx213(任务id)
