...
-- @Last Modified time: 2025-01-09 23:11:20
-- @作者: baidwwy
-- @邮箱:  313738139@qq.com
-- @创建时间:   2021-02-01 22:33:14
-- @最后修改来自: baidwwy
-- @Last Modified time: 2023-02-10 16:53:49
function 设置任务501(id)
	if 玩家数据[id].角色:取任务(13)~=0 then 添加最后对话(id,"人物修炼和召唤兽修炼不能同时进行")return end
	if 玩家数据[id].角色:取任务(501)~=0 then 添加最后对话(id,"你已经有人物修炼了任务了")return end
	...
	玩家数据[id].角色:扣除银子(50000,0,0,"领取人物修炼任务",1)
...
function 完成任务_501(id,任务id)
	...
	玩家数据[id].角色:添加人物修炼经验(id,积分)
	-- 发送数据(玩家数据[id].连接id,38,{内容="你获得了"..积分.."点人物修炼经验"})
	if 玩家数据[id].队伍 ~= 0 then
		for n=1,#队伍数据[玩家数据[id].队伍].成员数据 do
			local id1 = 队伍数据[玩家数据[id].队伍].成员数据[n]
			if id1 ~= id then
				local 积分1 = qz(积分)*10--获得经验倍率
				玩家数据[id1].角色:添加人物修炼经验(id1,积分1)
				-- 玩家数据[id1].角色:添加宠物修炼经验(id1,积分1)

				-- 发送数据(玩家数据[id1].连接id,38,{内容="你获得了"..积分1.."点人物修炼经验"})
			end
		end
	end
	...
	if 任务数据[任务id].次数>=100 then
		...
		-- 广播消息({内容=format("#S(人物修炼)#R/%s#Y在人物修炼任务中的积分达到了100，因此获得了马真人奖励的#G/%s#Y",玩家数据[id].角色.名称,奖励数据[1]),频道="xt"})
		local 链接 = {提示=format("#S(人物修炼)#G/%s#W在#R人物修炼#W任务中的完成了100环任务，获得",玩家数据[id].角色.名称),频道="hd",结尾="#W以资鼓励！#85"}
		玩家数据[id].道具:给予超链接书铁(id,{15,15},nil,链接)
		玩家数据[id].角色:取消任务(任务id)
		常规提示(id,"#Y恭喜你完成了全部的人物修炼任务")
		任务数据[任务id].次数=0
		任务数据[任务id].积分=0
		任务数据[任务id]=nil
	else
...
function 任务说明501(玩家id,任务id)
	local 说明 = {}
	if 任务数据[任务id].分类==11 then
		说明={"任务链任务",format("#L找到%s的%s,当前第%s环，任务积分%s分。",任务数据[任务id].地图名称,任务数据[任务id].名称,任务数据[任务id].次数,任务数据[任务id].积分)}
	elseif 任务数据[任务id].分类==12 or 任务数据[任务id].分类==13 or 任务数据[任务id].分类==14 then
		说明={"任务链任务",format("#L将#Y/qqq|%s*物品查询*/%s#L交给%s的%s,当前第%s环，任务积分%s分。",任务数据[任务id].物品,任务数据[任务id].物品,任务数据[任务id].地图名称,任务数据[任务id].名称,任务数据[任务id].次数,任务数据[任务id].积分)}
	elseif 任务数据[任务id].分类==15 then
		说明={"任务链任务",format("#L将#Y%s#L交给%s的%s,当前第%s环，任务积分%s分。",任务数据[任务id].bb,任务数据[任务id].地图名称,任务数据[任务id].名称,任务数据[任务id].次数,任务数据[任务id].积分)}
	end
	return 说明
end