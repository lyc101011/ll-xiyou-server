...
__NPCdh111[1145]=function (ID,编号,页数,已经在任务中,数字id)
	local wb = {}
	local xx = {}
	if 编号 == 1 then
    ...
    elseif 玩家数据[数字id].角色.门派~="魔王寨" then
		...
		return {"牛魔王","牛魔王",wb[sj(1,#wb)],xx,"门派师傅"}
	end
	return
end

__NPCdh222[1145 ]=function  (id,数字id,序号,内容)
	local 事件=内容[1]
  	local 名称=内容[3]
  	if 名称=="牛魔王" then
        local 门派类型="魔王寨"
