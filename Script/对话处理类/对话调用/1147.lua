...
__NPCdh111[1147]=function (ID,编号,页数,已经在任务中,数字id)
	local wb = {}
	local xx = {}
	if 编号 == 1 then
		...
		return {"镇元子","镇元子",wb[sj(1,#wb)],xx,"门派师傅"}
	end
	return
end

__NPCdh222[1147 ]=function  (id,数字id,序号,内容)
	local 事件=内容[1]
  	local 名称=内容[3]
  	if 名称=="镇元子" then
        local 门派类型="五庄观"
