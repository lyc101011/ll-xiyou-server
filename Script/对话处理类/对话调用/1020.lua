...
__NPCdh111[1020]=function (ID,编号,页数,已经在任务中,数字id)
	local wb = {}
	local xx = {}
	if 编号 == 1 then
		...
		local xx = {"购买","我只是来看看"}
		return{"男人_老孙头","武器店掌柜",wb[sj(1,#wb)],xx}
	elseif 编号 == 2 then
		wb[1] = "客官想要什么兵器？"
...
__NPCdh222[1020 ]=function  (id,数字id,序号,内容)
	local 事件=内容[1]
  	local 名称=内容[3]
  	if 名称=="武器店老板" then  --建邺
        if 事件=="购买" then
          玩家数据[数字id].商品列表=商店处理类.商品列表[8]
          发送数据(id,9,{商品=商店处理类.商品列表[8],银子=玩家数据[数字id].角色.银子})
        end
